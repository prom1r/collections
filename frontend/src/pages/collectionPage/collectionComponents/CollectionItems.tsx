...
export const CollectionItems = (props) => {
    ...
    const onSent = async () => {
        ...
        if (dateFrom == null && dateTo == null) {
            ...
            await getMyItems(props.collectionId, date, dateFrom.toISOString(), '').then((result) => {
                setMyItems(result)

            })
        } else if (dateFrom == null && dateTo) {
            await getMyItems(props.collectionId, date, '', dateTo.toISOString()).then((result) => {
                setMyItems(result)
            })
        } else {
            await getMyItems(props.collectionId, date, dateFrom.toISOString(), dateTo.toISOString()).then((result) => {
                setMyItems(result)
            })
        }
