...
const getItemsForm = async (
  ...
  if (dateFrom && dateTo == "") {
    dateFilerFrom["$gte"] = new Date(dateFrom);
    myItemsFrom = await Item.find({
      $and: [{ collectionId: collectionId }, { date: dateFilerFrom }],
    })
      .sort({ date: sort })
      .exec();
    return myItemsFrom;
  ...
  if (dateFrom == "" && dateTo) {
    dateFilerTo["$lte"] = new Date(dateTo);
    myItemsFrom = await Item.find({
      $and: [{ collectionId: collectionId }, { date: dateFilerTo }],
    })
      .sort({ date: sort })
      .exec();
    return myItemsFrom;
...
const deleteItem = async (id, collectionId) => {
  await Collection.updateOne(
    { _id: collectionId },
    { $inc: { itemCount: -1 } }
  );
